...
Retrieves all charm level definitions, providing threshold requirements and associated badge references for the progression system.

### Responsibilities

- Return all active charm level configurations
...
| Cache Key         | `levels:charm:definitions`                   |
...
{
  "status": "success",
  "message": "Charm levels retrieved",
  "data": {
    "levels": [
      {
        "level": 1,
  ...
  "meta": {
    "timestamp": "2026-02-01T18:23:06.000000Z",
    "correlation_id": "uuid"
  }
}
```
...
│ 3.1 ENTRY POINT                                                             │
│─────────────────────────────────────────────────────────────────────────────│
│ File: routes/api/levels.php:19                                              │
│ Route: Route::get('/charm', [LevelController::class, 'charmLevels'])        │
│                                                                             │
│ ┌─────────────────────────────────────────────────────────────────────────┐ │
│ │ Route::prefix('levels')->group(function () {                            │ │
│ │     Route::get('/charm', [LevelController::class, 'charmLevels'])       │ │
│ │         ->name('levels.charm');                                         │ │
│ │ });                                                                     │ │
│ └─────────────────────────────────────────────────────────────────────────┘ │
│                                                                             │
...
│─────────────────────────────────────────────────────────────────────────────│
│ File: app/Http/Controllers/Api/V1/Progression/LevelController.php:58        │
│                                                                             │
...
│ └─────────────────────────────────────────────────────────────────────────┘ │
│                                                                             │
│ STEP 3: Extract charm_levels and return via ApiResponse                     │
│ ┌─────────────────────────────────────────────────────────────────────────┐ │
│ │ return ApiResponse::success([                                           │ │
│ │     'levels' => $config['charm_levels'],                                │ │
│ │ ], 'Charm levels retrieved');                                           │ │
│ └─────────────────────────────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
...
│ │   • scopeCharm() → shortcut filter for charm type                       │ │
...
│ CACHE OPERATIONS:                                                           │
│                                                                             │
│ 1. GET: "levels:charm:definitions" (TTL: 86400 seconds / 24 hours)          │
...
│                                                                             │
│ 1. SELECT: Level definitions for charm type                                 │
│    ┌─────────────────────────────────────────────────────────────────────┐  │
...
│ getLevelConfiguration() implementation, but only returns charm.             │
...
| Case                         | Behavior                                         |
| ---------------------------- | ------------------------------------------------ |
| No charm levels in database  | Returns empty `levels` array                     |
...
| Additional metadata         | `LevelController::charmLevels()` response object   |
...
| Property            | Value                      |
| ------------------- | -------------------------- |
| **Endpoint**        | `GET /api/v1/levels/charm` |
| **Domain**          | Progression                |
