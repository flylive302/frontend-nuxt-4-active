 ...
 * logged, never surfaced. Both flips that drive this are foreground,
 * user-initiated actions — entering a room (`consuming`) and taking a Seat
 * (`producing`) — so every service START happens in the foreground and satisfies
 * the Android 14+ "no background start" rule. A backgrounded demotion only ever
 * STOPS `microphone` (the `mediaPlayback` service stays up), never starting a
 * service from the background (capacitor-04 D1).
 */
