...
export function useVoicePlayback(messageId: string, fallbackDurationMs: number) {
  ...
  const effectiveDurationMs = computed(() => (isPlaying.value && durationMs.value > 0) ? durationMs.value : fallbackDurationMs)
  const currentPositionMs = computed(() => isPlaying.value ? positionMs.value : 0)
